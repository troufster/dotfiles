...
while read -r line ; do
  case $line in
    ...
    BAT*)
      bat="${line#???}"
      batl="%{F${color_sec_b3}}${sep_left}%{F${color_icon} B${color_sec_b3}} %{T2}${icon_bat}%{F- T1} ${bat}"
      ;;
    VOL*)
      # Volume
    ...
    MPD*)
      ...
      mpd="%{F${color_sec_b2}}${sep_left}%{F${color_icon} B${color_sec_b2}}  %{T2}${icon_music}%{F${color_fore} T1}  ${song}"
      ;;
    WSP*)
      # I3 Workspaces
  ...
  # And finally, output
  printf "%s\n" "%{l}${wsp}${title} %{r}${mpd}${stab}${irc}${stab}${gmail}${stab}${diskr}${stab}${diskh}${stab}${vol}${stab}${batl}${stab}${date}${stab}${time}"
  #printf "%s\n" "%{l}${wsp}${title}"
done
